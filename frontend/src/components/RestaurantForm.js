...
export default class RestaurantForm extends React.Component {
    ...
    handleDetailsUpdate = async (e) => {
        ...
        const connection = await fetch("http://localhost:8000/restaurantdetails", {
            method: "POST",
            body: JSON.stringify({ name: event.target.name.value, address: event.target.address.value, description: event.target.description.value, email: event.target.email.value, phone: event.target.phone.value }),
    ...
    render() {
        ...
        const { Name, Description, Address, Email, Phone } = this.state.restaurantData;
